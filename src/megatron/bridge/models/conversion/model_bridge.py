        ...
        for task in self._with_progress_tracking(megatron_to_hf_tasks, "Converting to HuggingFace", show_progress):
            ...
            if getattr(task.mapping, "is_grouped_export", False):
                ...
                if merged_result is not None:
                    for hf_name, tensor in merged_result.items():
                        yield HFWeightTuple(hf_name, tensor.cpu() if cpu else tensor, task.global_param_name)
                continue
            ...
            for hf_name, tensor in converted_weights_dict.items():
                ...
                if embeddings_are_tied and hf_name == "model.embed_tokens.weight":
                    ...
                    yield HFWeightTuple(hf_name, final_tensor, task.global_param_name)
                    ...
                    if hasattr(hf_pretrained, "state") and hasattr(hf_pretrained.state, "source"):
                        ...
                        if "lm_head.weight" in expected_keys:
                            yield HFWeightTuple("lm_head.weight", final_tensor.clone().detach(), task.global_param_name)
